...
*Prepared 1 August 2026. Toy / educational model — not a trading or investment tool.*
...
*Artifacts: data pipeline `build_ng_dataset_monthly.py` → `NG_m_final.csv`; deployable model `ttf_european_model.py`; conditional perfect-foresight model `ttf_european_conditional.py`; endogeneity-free ceiling `ttf_european_exogenous.py`; lean robustness check `ttf_european_lean.py`.*