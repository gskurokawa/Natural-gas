...
def load_supplementary_excel() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    """
    path = INPUT_DIR / "European_natural_gas_-_SUPPLEMENTARY_DATA.xlsx"
...
def add_derived_variables(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Piped"] = sum(df[f"bcm_{c}"] for c in pipeline_corridors if f"bcm_{c}" in df.columns)

    df["Production(bcm)"] = pd.to_numeric(df["Production(bcm)"], errors="coerce")
    df["Net_supply"] = (
        df["LNG_sendout(bcm)"] + df["Production(bcm)"] + df["Piped"]
        - df["CH_RS_net_exports"] - df["bcm_ES"] - df["Distribution_loss(bcm)"]
    )
    ...
    # Simple geopolitical-risk tiers based on the PRIOR month's TTF level.
    # (Deliberately crude and price-derived -- treat as a proxy, not a
    # genuine exogenous indicator; see the accompanying model report for
    # why a variable built this way should not be used as a predictor.)
    df["geo"] = 0
    df.loc[df["TTF(USD/mmbtu)"].shift(1) >= 20, "geo"] = 1
    df.loc[df["TTF(USD/mmbtu)"].shift(1) >= 30, "geo"] = 2
    df.loc[df["TTF(USD/mmbtu)"].shift(1) >= 40, "geo"] = 3
