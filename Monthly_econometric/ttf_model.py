...
A toy econometric model for forecasting month-ahead TTF (European natural
...
Pipeline:
  ...
  2. Engineer deseasonalized ("anomaly") predictors: momentum, storage
     change, LNG sendout, and London heating-degree-days -- each compared
     only to its own expanding, prior-years-only calendar-month average, so
     no predictor ever uses future information.
  3. Fit OLS with Newey-West HAC standard errors.
  4. Run regression diagnostics: Durbin-Watson, residual autocorrelation,
     Breusch-Pagan heteroskedasticity test.
  5. Backtest out-of-sample with an expanding-window walk-forward loop,
     benchmarked against a random walk, a momentum-only model, and the
     historical mean.
  6. Repeat steps 2-5 for 1-, 2-, and 3-month-ahead horizons.
  7. Produce the two summary charts used in the accompanying report.
...
def build_features(df: pd.DataFrame, horizon: int = 1) -> pd.DataFrame:
    """
    Build the target and the four deseasonalized, horizon-lagged predictors
    ...
    relative to the return they forecast):
      ...
      - LNG               : deseasonalized log LNG sendout
    ...
    log_lng = np.log(df["LNG_sendout(bcm)"])
    out["LNG_anom"] = (
        log_lng - expanding_seasonal_mean(log_lng, df["month"])
    ).shift(horizon)
...
FEATURE_COLS = ["momentum", "storage_change_anom", "LNG_anom", "HDD_anom"]
