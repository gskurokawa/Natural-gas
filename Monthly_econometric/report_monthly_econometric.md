...
| Log LNG sendout, level | -1.74 | -2.86 | 1.959 | 0.463 | **Non-stationary** |
| LNG sendout anomaly (deseasonalized level) | -2.49 | -2.86 | 0.760 | 0.463 | **Still non-stationary** |

**LNG sendout was dropped from the model as a result of this check.** Deseasonalizing by calendar month removes seasonal variation, but does nothing about a slower-moving structural trend — and EU+UK LNG sendout has exactly that, having grown substantially and persistently since 2022 as Europe replaced lost Russian pipeline volumes and, more recently, absorbed rising US LNG export capacity. Both ADF and KPSS agree the raw level is non-stationary, and critically, both still agree the deseasonalized anomaly is non-stationary too. As a direct check, a growth-rate version (month-over-month % change rather than level, which is stationary by construction: ADF t=-12.24, KPSS=0.088) was substituted into the regression, and its coefficient became statistically indistinguishable from zero (p=0.55, versus p=0.02 for the level version). This indicates that LNG's apparent significance in an earlier version of this model was likely driven at least in part by both series drifting together over the same structural window, rather than reflecting a genuine, stable month-to-month relationship — a textbook risk of regressing on a non-stationary predictor. LNG sendout was therefore excluded from the final specification below.
...
All three diagnostics are clean. Residual autocorrelation is essentially absent, and — notably, this improved after removing LNG sendout — the Breusch-Pagan test now comfortably fails to reject homoskedasticity, whereas an earlier version of the model that included the (non-stationary) LNG variable showed borderline heteroskedasticity. Newey-West HAC standard errors are still reported throughout for robustness, but this model's residuals are well-behaved by conventional OLS standards too.
...
Storage change anomaly is the only variable that remains significant at the 5% level across all three horizons (p=0.018, 0.019, 0.049); momentum and HDD are significant or marginal at H=1 but not at H=2/H=3. Out-of-sample R² relative to the random walk still improves with horizon, echoing the pattern seen with the (now-excluded) LNG-inclusive model, but the loss-differential t-statistic no longer reaches conventional significance at any horizon tested — a materially weaker result than the LNG-inclusive version previously suggested, and one that better reflects a model built on three variables that survived scrutiny rather than four, one of which did not. As before, a placebo check using pure random noise in an identically-constructed overlapping regression shows R² rising mechanically with horizon too (the Boudoukh–Richardson–Whitelaw overlapping-regression bias), so the horizon results here should be read as **suggestive at best, not confirmatory** — broadly consistent with a modest, gradually-diffusing fundamentals effect, but not independent statistical proof of one.
...
The general principle — that physical flow/balance data diffuses into price gradually, while weather is priced in close to instantly and then partially reverses — is a reasonable economic prior independent of this dataset, given how each type of data is generated and reported. It was also the reasoning that motivated testing an LNG sendout variable, which showed the same continuation signature as storage change; that variable was ultimately dropped from the model on stationarity grounds (Section 6), not because the economic logic was wrong, but because the specific series available here was not statistically sound to use as constructed.
...
**The constant** is statistically indistinguishable from zero (p=0.703). This is a cleaner result than an earlier LNG-inclusive version of this model, where the constant was significant and appeared to be absorbing part of LNG's non-stationary trend — with LNG removed, there is no significant residual drift left for the model to explain once storage change, HDD, and momentum are accounted for.
...
- An LNG sendout variable was tested and initially appeared to be one of the stronger predictors in the model, but was excluded after formal stationarity testing showed both the raw series and its deseasonalized anomaly failed to reject a unit root (Section 6). This is a useful general caution for this kind of exercise: with many candidate fundamentals and a small sample, it is easy for a non-stationary series that happens to trend alongside the dependent variable over one structural period (here, the post-2022 European energy crisis and subsequent LNG capacity buildout) to look like a genuine predictor when it is not. A growth-rate version of LNG sendout was stationarity-clean but statistically insignificant, and was not included in the final model either, since retaining an insignificant regressor only dilutes degrees of freedom in an already small sample.
...
- A revisited, properly stationary treatment of LNG-related supply data (e.g. a capacity-utilization ratio rather than a raw sendout level, which may be better behaved than either the level or simple growth-rate versions tested here) is a plausible direction for future work, particularly given the ongoing structural growth in global LNG export capacity — this has been falling as a constraint on price due to the rapid increase in US LNG export capacity, and likely contributed to depressed TTF prices over winter 2025/2026. An international crisis dummy, or a regime-dependent switching model (e.g. conditioned on the VIX or Brent crude), is another plausible extension not attempted here.
