...
data = data.drop(columns=['US_ETF2x', 'US_iETF2x', 'US_ETF', 'TTF(EUR)'])
...
x = pd.read_csv('TTF_prices_Jan15-Sep17.csv')
x['TTF'] = x['TTF'] * data['EURUSD=X'] / 3.412
data['TTF'] = data['TTF'].fillna(x['TTF'])
data = data.drop(columns=['EURUSD=X'])
data['JKM'] = data['JKM'].fillna(7.86) # From research
...
storage_df = storage_df.rename(columns={'gasDayStart': 'Date', 'gasInStorage': 'Av_storage(bcm)'})
storage_df['Av_storage(bcm)'] = pd.to_numeric(storage_df['Av_storage(bcm)'], errors='coerce')
storage_df['Av_storage(bcm)'] = storage_df['Av_storage(bcm)'] / 10.5

storage_df['Date'] = pd.to_datetime(storage_df['Date'])
storage_df = storage_df.sort_values('Date')
print(storage_df)
storage_df.to_csv('test_storage.csv', index=False)


# %%

x = pd.read_csv('NG_monthly0.csv')
x = pd.merge(data, storage_df, on='Date', how='left')
...
print(lng.head(20))
print(lng.tail(20))
x = pd.read_csv('NG_monthly1.csv')
x['Date'] = pd.to_datetime(x['Date'])
lng['LNG_sendout(GWh/d)'] = lng['LNG_sendout(GWh/d)'].interpolate(method='linear')
x = pd.merge(x, lng, on='Date', how='outer')
x.loc[0, 'LNG_sendout(GWh/d)'] = 1200
x.loc[1, 'LNG_sendout(GWh/d)'] = 1200
print(x)
...
x = x.drop(columns=['Battery', 'Battery Consumption'])
...
x = pd.read_csv('NG_monthly6.csv')
x['Date'] = pd.to_datetime(x['Date']).dt.tz_localize(None)
y = pd.read_csv('eurostat_gas_balance_monthly.csv')
y['Date'] = pd.to_datetime(y['Date']).dt.tz_localize(None)
merge = pd.merge(x, y, on='Date', how='left')
print(merge)
merge.to_csv('NG_monthly7.csv', index=False)


# %% Compile and finalize all data

y = pd.read_excel('European natural gas - SUPPLEMENTARY DATA.xlsx', sheet_name='Transform')
...
x = x.drop(columns=['Final_Consumption_Industry(MIO_M3)'])
x.to_csv('NG_monthly8.csv', index=False)
...
df = pd.concat([z1, z2, z3, z4, z5, z6], axis=0, ignore_index=True)
...
w1 = pd.read_csv('europe_pipeline_exports_detailed_2021.csv')
w2 = pd.read_csv('europe_pipeline_exports_detailed_2022.csv')
w3 = pd.read_csv('europe_pipeline_exports_detailed_2023.csv')
w4 = pd.read_csv('europe_pipeline_exports_detailed_2024.csv')
w5 = pd.read_csv('europe_pipeline_exports_detailed_2025.csv')
w6 = pd.read_csv('europe_pipeline_exports_detailed_2026.csv')

w1.columns = w1.columns.str.strip()
w2.columns = w2.columns.str.strip()
w3.columns = w3.columns.str.strip()
w4.columns = w4.columns.str.strip()
w5.columns = w5.columns.str.strip()
w6.columns = w6.columns.str.strip()

df2 = pd.concat([w1, w2, w3, w4, w5, w6], axis=0, ignore_index=True)
df2 = df2.rename(columns={'date':'Date'})
df2['Date'] = pd.to_datetime(df2['Date'], format='mixed', dayfirst=True).dt.tz_localize(None)
...
df.to_csv('NG_europe_pipelines_Feb2021_onwards.csv', index=False)
print(df)

clist = ['Date', 'corridor_BY', 'corridor_DE', 'corridor_DZ', 'corridor_LY', 'corridor_MA', 'corridor_NO', 'corridor_RU-Baltic', 'corridor_TR', 'corridor_UA', 'corridor_ES']

df = df[clist]
df['Date'] = pd.to_datetime(df['Date']).dt.strftime('%Y-%m-%d')
x = pd.read_csv('NG_europe_pipelines_2015-2020.csv')
x['Date'] = pd.to_datetime(x['Date'], format='%b-%y').dt.strftime('%Y-%m-%d')
x = pd.concat([x, df], axis=0)
x.to_csv('NG_europe_pipelines.csv', index=False)

# %% Concat

x = pd.read_csv('NG_monthly8.csv')
y = pd.read_csv('NG_europe_pipelines.csv')
merge = pd.merge(x, y, on='Date', how='left')

# Post-merge unit transformations
merge.columns = merge.columns.str.replace('corridor', 'bcm', regex=False)
bcmlist = ['BY', 'DZ', 'LY', 'MA', 'NO', 'RU-Baltic', 'TR', 'UA', 'DE']
for i in bcmlist:
    merge["bcm_" + i] = pd.to_numeric(merge["bcm_" + i], errors='coerce')
    merge["bcm_" + i] = merge["bcm_" + i] / 10500 # Unit conversion GWh to bcm
merge['Date'] = pd.to_datetime(merge['Date'])
merge['Days'] = merge['Date'].dt.days_in_month
merge['LNG_sendout(bcm)'] = merge['LNG_sendout(GWh/d)'] * merge['Days'] / 10500
merge = merge.drop(columns=['LNG_sendout(GWh/d)'])
merge.to_csv('NG_monthly9.csv', index=False)
