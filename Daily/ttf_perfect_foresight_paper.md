...
Baumeister, Huber, Lee and Ravazzolo (2024) show that statistically and economically meaningful results can be obtained the Henry Hub price. In a structural VAR of the German gas market, Nick and Thoenes (2014) identify storage levels, temperature-driven demand and supply disruptions as the principal drivers of gas prices. A parallel machine-learning literature (e.g. Čeperić, Žiković and Čeperić, 2017) reports that nonlinear models coupled with feature-selection procedures can improve short-term accuracy, but the documented improvements are typically modest and vulnerable to overfitting. Linear and ARIMA-type specifications remain competitive benchmarks. 

The Dutch TTF has become the reference price for a globally integrated LNG market, so global conditions increasingly bear on it. Obadi and Korček (2020) obtain a high in-sample fit (R² ≈ 0.82) for month-ahead TTF, but the explanatory power is carried by German power and coal prices. However, the latter are co-determined with gas through power-sector switching. 

In contrast to the difficulty forecasting price levels, the volatility of natural gas prices is forecastable. Corsi (2009) forecast gas volatility out of sample using heterogeneous autoregressive (HAR) models. Kristjanpoller (2024) confirms that natural gas realized volatility is predictable with HAR and machine learning models and that adding exogenous financial variables (EUR/USD, Brent, equity indices). Ergen and Rizvanoghlu (2016) and Liang, Xia, Lai and Wang (2022) find that GARCH frameworks respond to storage levels as well as temperature deviations and extreme weather. Much of the forecastability is the autoregressive persistence of volatility itself, with fundamentals adding at the margin. 
